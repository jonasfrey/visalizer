...
o_parser = argparse.ArgumentParser(
    description="Classify images using a pretrained YOLO classification model. Outputs top-5 ImageNet class predictions per image via IPC."
)
o_parser.add_argument("a_s_path", nargs="+", help="Paths to image files to classify")
o_parser.add_argument("--s-uuid", default=s_uuid__default, help="S_UUID for IPC output (default from env)")
o_parser.add_argument("--n-top", type=int, default=5, help="Number of top predictions per image (default: 5)")
...
print(f"  | --n-top             {str(n_top):30s}             |")
print("  +------------------------------------------------------+")

# --- 3. load model ---

o_timer__model = f_time_start("load_yolo_model")
f_log("Loading YOLO classification model...")
o_model = YOLO(".gitignored/yolov8m-cls.pt")
...
for s_path in a_s_path:
    ...
    o_timer__classify = f_time_start(f"classify {os.path.basename(s_path)}")
    a_o_prediction_result = o_model(s_path, verbose=False)
    f_time_end(o_timer__classify)

    a_o_prediction = []
    for o_r in a_o_prediction_result:
        a_n_top_idx = o_r.probs.top5[:n_top]
        a_n_top_conf = o_r.probs.top5conf[:n_top]
        o_names = o_r.names

        for n_rank, (n_idx, n_conf) in enumerate(zip(a_n_top_idx, a_n_top_conf), start=1):
            n_idx = int(n_idx)
            n_conf = float(n_conf)
            s_name = o_names[n_idx]
            a_o_prediction.append({
                'n_index': n_idx,
                's_name': s_name,
                'n_confidence': round(n_conf, 6),
                'n_rank': n_rank,
            })
            f_log(f"  rank {n_rank}: {s_name:20s} {n_conf:.2%} (index {n_idx})")

    a_o_result.append({
        's_path_absolute': os.path.abspath(s_path),
        'a_o_prediction': a_o_prediction,
    })
